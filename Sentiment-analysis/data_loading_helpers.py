...
class new_data_box:
    ...
    def __init__(self, input_config = new_configs.default_config, seed = 100):
        ...
        print(self.input_config['SUBJECTS'])
        for subject in self.input_config['SUBJECTS']:
            print(subject)
            ...
            if self.input_config["EEG_SIGNAL"]:
                self.sentence_level_data['eeg_signals'] = np.array(self.sentence_level_data['eeg_signals'])
    ...
    def _load_word_level_subject(self, subject_data):
        ...
        for sentence in subject_data.keys():
            ...
            if self.input_config["EEG_SIGNAL"] == True:
####################################### input data #######################
                eeg = np.array(self._extract_word_level_eegs(word_level_data))
                
                self.sentence_level_data['eeg_signals'].append(eeg)
                print(self.sentence_level_data['eeg_signals'].shape)
        ...
        if self.input_config["EEG_SIGNAL"] == True and compress_eeg == True:
            self.sentence_level_data['eeg_signals'] = np.array(sentence_level_data['eeg_signals'])
        ...
        if self.input_config["EEG_SIGNAL"] == True:
            print("##################### check_before_sentence_level_data ###########################")
            print(self.sentence_level_data['eeg_signals'][0])
            print(self.sentence_level_data['eeg_signals'][0].shape)
    ...
    #1. load mdl & set property
        with open("./Data_to_preprocess/2class_mdl.pkl", "rb") as f:
            mdl = pkl.load(f)

        fb = [4.0, 50.0] #lowest , Highest frequency band
        cc = 0.5
        fs = 500.0
        step = 10 #this time, we will slice 80 for window 
        ...
        for word_idx in word_level_data.keys():
            if type(word_idx) == int:
                ...
                for fixation in word_level_data[word_idx][eeg_format].keys():
                    try :
                        eeg = word_level_data[word_idx][eeg_format][fixation] # wordlevel, (105, Time)
                        print("eeg shape : ",eeg.T.shape)
                    ############# need to change this function ####################
                        if not is_nan_or_none(eeg):
   #3. calculate fcg    
                            fcg = tvfcg(np.squeeze(eeg.T), estimator, fb, fs, cc, step)
   ...
   #4. Using mdl, encode
                            ...
                            print("symbols shape : ", symbols.shape) #symbols : (# of fcg, 1)

                            fixation_eegs.append(np.array(symbols)) #original_input(eeg) : (105, time)
                        ...
                        print("window is too large for this word")
                        fixation_eegs.append([[99]])
...
def load_all_word_eegs(subject, eeg_format = 'RAW_EEG'):
    subject_data_path = './Data_to_preprocess/Sentence_label_data/Sentence_data_' + subject + '.pickle'
    with open(r'{}'.format(subject_data_path), 'rb') as subject_file:
        subject_data = pkl.load(subject_file)
        ...
        for word_idx in word_level_data.keys():
            if type(word_idx) == int:
                ...
                for fixation in word_level_data[word_idx][eeg_format].keys():
                    ...
                    if not is_nan_or_none(eeg):
   #3. calculate fcg    
                        fcg = tvfcg(np.squeeze(eeg), estimator, fb, fs, cc, step)
                        fcgs = np.array(np.real(fcg))
                        print("1 st fcgs shape : ",fcgs.shape)
                        triu_ind = np.triu_indices_from(np.squeeze(fcgs[0, ...]), k=1)
                        fcgs = fcgs[:, triu_ind[0], triu_ind[1]]
                        print("2 nd fcgs shape : ", fcgs.shape)
   #4. Using mdl, encode
                        encoding, symbols = mdl.encode(fcgs)
                        print("symbols shape : ", symbols.shape) #symbols : (# of fcg, 1)
